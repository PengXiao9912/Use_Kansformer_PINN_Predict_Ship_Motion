...
import random
from PinnLoss import MMG_PINN_Loss
...
def main(data_dir, save_dir='./'):
    ...
    for epoch in range(EPOCHS):
        model.train()
        t_data_loss = t_pde_loss = t_total_loss = 0.0
        for xb, yb in train_loader:
            ...
            loss_u, loss_v, loss_r, pde_loss = pinn_criterion(xb, out)  
            loss = data_loss + pinn_loss_weight * pde_loss
        ...
        model.eval()
        v_data_loss, v_pde_loss, v_total_loss = 0, 0, 0
        for xb, yb in val_loader:
            xb, yb = xb.to(DEVICE), yb.to(DEVICE)
            # —— 1. 先在 no_grad 下做模型推断并算 data_loss —— 
            with torch.no_grad():
                out, _ = model(xb)
                data_loss = criterion(out, yb)
            xb.requires_grad_(True)
            with torch.enable_grad():
                out, _ = model(xb)
                loss_MMG_x, loss_MMG_y, loss_MMG_N, pde_loss = pinn_criterion(xb, out)
            total_loss = data_loss + pinn_loss_weight * pde_loss
        ...
        print(f"[Epoch {epoch+1:03d}] "
              f"Train: Data={t_data_loss:.6f}, PDE={t_pde_loss:.6f}, Total={t_total_loss:.6f} | "
              f"Val: Data={v_data_loss:.6f}, PDE={v_pde_loss:.6f}, Total={v_total_loss:.6f}")

        # === 保存 loss history ===
        loss_hist.append({
            'epoch': epoch+1,
            'train_data': t_data_loss,
            'train_pde': t_pde_loss,
            'train_total': t_total_loss,
            'val_data': v_data_loss,
            'val_pde': v_pde_loss,
            'val_total': v_total_loss,
        })
        # === 保存最佳模型 ===
        if v_total_loss < best:
            ...
            torch.save({
                ...
                's_in': scaler_input,
                's_tg': scaler_target,
            }, os.path.join(save_dir, 'kan+transformer+pinn_best_model.pth'))


    pd.DataFrame(loss_hist).to_csv(os.path.join(save_dir, 'kan+transformer+pinn_loss_history.csv'), index=False)
